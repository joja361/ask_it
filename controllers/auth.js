  ...
  try {
    ...
    if (err.code === "23505") {
      const error = new Error(
        "This email is already taken, please try with another."
      );
      error.statusCode = 422; // 422 Unprocessable Entity
      return next(error);
    }
...
export const login = async (req, res, next) => {
  ...
  let user;
  const text = "SELECT id, email, password, name FROM account WHERE email=$1";
  try {
  ...
  res.send({ userId: user.id, token, email, name: user.name });
};
...
export const resetPassword = async (req, res, next) => {
  ...
  if (!passwordMatch) {
    const error = new Error(
      "You entered incorrect old password, please try again."
    );
    error.statusCode = 401; // 401 Unauthorized
    return next(error);
  }
